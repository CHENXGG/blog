...
根据窄带信号的特点，我们可以采取正交解调的方式，取出包络的实部和虚部，然后进行DFT即可获得包络$\tilde{\beta}\tilde{x}(t-\tau_a)\mathrm{exp}(j2\pi f_d\tau_a)$的相关频谱。
...
$$
\begin{aligned}
L(\tau, f) & \approx \frac{2\sqrt{E_r}}{N_0}|\tilde{y}(t)| \\
		   & = \frac{2\sqrt{E_r}}{N_0}|\tilde{x}(t)*\tilde{h}(t)| \\	
